...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    super.initState();
    context.read<DetailMovieBloc>().add(FetchDetailsMovieEvent(widget.id));
    context.read<WatchlistMoviesBloc>().add(FetchStatusMoviesEvent(widget.id));
    context
        ...
        listener: (_, state) {
          if (state is AddToWatchlistMoviesSuccess) {
            ...
            context
                ...
                .add(FetchStatusMoviesEvent(widget.id));
          }
        },
          ...
          builder: (context, state) {
            if (state is DetailMoviesLoading) {
              ...
              bool isAddedToWatchlist = (context
                      ...
                      .state is WatchlistStatussMovieLoaded)
                  ? (context.read<WatchlistMoviesBloc>().state
                          as WatchlistStatussMovieLoaded)
                      .result
                  : false;
              return SafeArea(
                child: DetailContent(
              ...
              return Text("No movie recommendation");
            }
          },
        ),
      ),
                              ...
                              onPressed: () async {
                                if (!isAddedWatchlist) {
                                  BlocProvider.of<WatchlistMoviesBloc>(context)
                                    ..add(AddToWatchlistMoviesEvent(movie));
                                } else {
                                  BlocProvider.of<WatchlistMoviesBloc>(context)
                                    ..add(
                                        RemoveFromWatchlistMoviesEvent(movie));
                                }
                              },
                              child: Row(
